...
class LargeWidget : AppWidgetProvider() {
    ...
    companion object {
        fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int, state: BatteryState) {
            ...
            val remainingText = if (state.isCharging) {
                when {
                    state.timeTo100Min > 0 -> "⏳ ${state.timeTo100Min}m to full"
                    state.timeTo80Min > 0 && state.percentage < 80 -> "⏳ ${state.timeTo80Min}m to 80%"
                    state.timeTo50Min > 0 && state.percentage < 50 -> "⏳ ${state.timeTo50Min}m to 50%"
                    else -> "⏳ Charging speed: ${String.format(Locale.US, "%.1f", state.speed)}%/h"
                }
            } else {
                if (state.speed > 0f) {
                    val remMinutes = ((state.percentage.toFloat() / state.speed) * 60).toInt()
                    val hours = remMinutes / 60
                    val mins = remMinutes % 60
                    if (hours > 0 || mins > 0) "⏳ ${hours}h ${mins}m remaining" else "⏳ Calculating remaining time..."
                } else {
